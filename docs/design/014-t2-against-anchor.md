...
| **G4-014** | Every negative case rejects for the **right** reason, with the right code: `W11`, `N2`, `N12`, `L11`, `L22` | A-6…A-9. **CLOSED, 2026-08-11**: `TestNegativePaths014` (`tests/receipt/test_anchored_app_live.py`), real and live against dev-mode algod. Correction found: the code strings never appear literally in algod's real rejection text (stripped TEAL comments) — each test instead proves it's the *right* assert by construction (every other input in the group is honest, so only the targeted check can fire). |
| **G5-014** | Nothing is stranded by a failed group: box absent and app balance unchanged after each negative case | A-6, A-7. **CLOSED, 2026-08-11**, same tests — box-absence and balance-unchanged confirmed by real `application_box_by_name`/`account_info` reads after each rejection. |
