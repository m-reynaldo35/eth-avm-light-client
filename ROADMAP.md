...
| M1 | BLS12-381 point codec & MSM/pairing wrapper | Not Started | — | scaffold | Compressed (Eth wire format) vs. uncompressed (AVM opcode) trust boundary; use real G2 map-to-curve cost (8150), not G1 (1950) | 2026-07-30 | Dispatch Opus design agent |
| M2 | On-chain optimized RLP decoder | Not Started | — | scaffold | Fix O(child-index) re-walk in spike's `RLP_ITEM_SUB`; add hex-prefix nibble decode + EIP-2718 type-byte stripping (neither existed); single-blob vs. data-source interface (gates M7) | 2026-07-30 | Can run in parallel with M1/M3 |
| M3 | SSZ Merkle branch verifier | Not Started | — | scaffold | New territory, not in spike; must measure sha256 AVM opcode cost from scratch; pin target fork(s) (Altair/Capella/Deneb field layout differs) | 2026-07-30 | Can run in parallel with M1/M2 |
